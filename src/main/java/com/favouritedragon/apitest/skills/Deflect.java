...
public class Deflect extends SkillActive {
    ...
    public void deflectProjectiles(EntityPlayer player) {
        AxisAlignedBB boundingBox = player.getEntityBoundingBox();
        Vec3d look = player.getLookVec();
        AxisAlignedBB box = new AxisAlignedBB(boundingBox.minX + look.xCoord, boundingBox.minY + look.xCoord,
                boundingBox.minZ + look.zCoord, boundingBox.maxX + look.xCoord, boundingBox.maxY + look.yCoord,
                boundingBox.maxZ + look.zCoord);
        List<Entity> projectiles = player.worldObj.getEntitiesWithinAABB(Entity.class, box);
        if (!projectiles.isEmpty()) {
            for (Entity projectile : projectiles) {
                if (TargetUtils.isTargetInSight(player, projectile)) {
                    if (projectile instanceof EntityArrow || projectile instanceof EntityFireball || projectile instanceof EntityThrowable
                            || projectile.isAirBorne && projectile.canBeCollidedWith() && projectile.canBePushed()) {
                        if (attacksParried < getMaxParries() && parryTimer > getParryDelay() && player.getHeldItemMainhand() == null) {
                            ++attacksParried; // increment after disarm check
                            player.playSound(SoundEvents.BLOCK_ANVIL_PLACE, 0.4F, 0.5F);
                            playMissSound = false;
